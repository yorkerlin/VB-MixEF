...
if checkGrad
    if numDiff
		...
		derivativeCheck(funObj,x,computeHessian+1,useComplex,varargin{:});
	end
end
...
if ~isempty(outputFcn)
    callOutput(outputFcn,x,'init',0,funEvals,f,[],[],g,[],max(abs(g)),varargin{:});
end
...
for i = 1:maxIter
    ...
    if verboseI
        fprintf('%10d %10d %15.5e %15.5e %15.5e\n',i,funEvals*funEvalMultiplier,t,f,max(abs(g)));
    end

    % Output Function
    if ~isempty(outputFcn)
        callOutput(outputFcn,x,'iter',i,funEvals,f,t,gtd,g,d,max(abs(g)),varargin{:});
    end
...
% Output Function
if ~isempty(outputFcn)
    callOutput(outputFcn,x,'done',i,funEvals,f,t,gtd,g,d,max(abs(g)),varargin{:});
end
