...
M = N;
callbackFun=@(iter,post_dist)plotFun(iter,post_dist,w1,w2,n,post,wmap,w_exact_vi,C_exact_vi,'g'); %plot
...
exp_cvi_general(lik_grad, prior_grad, callbackFun, nSamples, maxIters, init_m, init_P, ss_0, beta1, seed);


function [gMu_pri, gAlpha_pri, gSigma_pri] = prior_gauss_grad(mu,alpha,Prec,priorMean,priorVar)
  gSigma_pri = - diag(1./priorVar)/2;
  gMu_pri = - (mu-priorMean)./priorVar  - alpha./priorVar;
  gAlpha_pri = -2.*alpha./priorVar - (mu-priorMean)./priorVar;
end
...
function dummy=plotFun(iter,post_dist,w1,w2,n,post,wmap,w_exact_vi,C_exact_vi,color)
     ...
     cc = exp_gauss_contour(post_dist.mean, post_dist.covMat,post_dist.alpha);
     [xp,yp,zp] = C2xyz(cc);
     hold on;
     once=0;
     for j = find(zp>cutoff);
         if once==0
             p = plot(xp{j},yp{j},color,'linewidth',3, 'DisplayName','Exp-Gauss');
             plot_leg(2)=p;
             once=1;
         else
